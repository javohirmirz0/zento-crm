...
export function CommsBox({ leadId, phone }: { leadId: string; phone: string | null }) {
  ...
  return (
    <div className="card p-4">
      <div className="text-sm font-medium text-white">SMS yuborish</div>
      {!phone && <div className="mt-1 text-xs text-red-400">Telefon raqami kiritilmagan</div>}
      <textarea
      ...
      <div className="mt-2 flex items-center justify-between">
        <span className="text-xs text-ink-500">{message.length}/480</span>
        <button className="btn-primary" onClick={send} disabled={sending || !message.trim() || !phone}>
          {sending ? "Yuborilmoqda..." : "Yuborish"}
        </button>
      </div>
      {result && (
        <div className={`mt-2 rounded-lg px-3 py-2 text-sm ${result.ok ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}>
          {result.text}
        </div>
      )}
    </div>
